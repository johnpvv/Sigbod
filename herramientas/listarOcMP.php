...
$fechaact = date("Y-m-d");
$rs = mysql_query("SELECT est_nombre,est_est FROM est_mp", $link);
?>
<html>
    <head>
        ...
        <title>SigBod - Listar OC MP</title>
        <script>
            $(document).ready(function () {
                var consulta;
    ...
    <body class="fondo">        
        <table class="table2a">
            <tr>
                <td class="texto">Listar Ordenes de Compra desde Mercado Público</td>
            </tr>
            <tr>
                <td class="texto5b">Ingrese Orden de Compra a Buscar:
                    ...
                    <span class='caja_pading2'>Fecha Orden de Compra:
                        ...
                        Estado OC: <select name="est" id="est" class="caja_texto" required> 
                            <option value="" selected>Seleccionar una opción</option>
                            <?php
                            while ($row = mysql_fetch_array($rs)) {
                                echo"<option value=", $row["est_est"], ">", $row["est_nombre"], "</option>";
                            }
                            ?>
                        </select>
                    </span>&nbsp;&nbsp;&nbsp;&nbsp;
