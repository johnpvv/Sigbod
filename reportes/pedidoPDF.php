...
<html lang="es">
    ...
    <body style="margin: -0.3in -0.3in 0in -0.3in;">
        <?php
        if (is_int($id / 100)) {
        ...
        $sql1 = "SELECT * FROM movimiento, proveedores, usuarios, estado, unid_oper, bodegas WHERE (mov_id ='$id') AND usuario_rut=mov_usuario AND mov_rutprv=prov_rut AND mov_estado=estado_id AND usuario_institucion=uni_id AND bodega_id=usuario_direccion"; //agrupar en parentesis los campos para que se ejecute primero la accion
        ...
        echo '<tr><td class="td1"><img src="../img/logo1.jpg" alt="logo" width="50" height="58"/></td><td class="td5">Hospital Clinico San Borja Arriarán</td><td class="td2">Fecha Solicitud: ' . date("d/m/Y, h:i A", strtotime($row1["mov_fecha"])) . '</td></tr>';
        echo '</table>';
        ...
        echo '</table>';
        echo '<table class="table4">';
        echo '<tr><td colspan="2" class="texto5" style="text-align:left;">1.- Datos Generales del Proveedor:</td></tr>';
        echo '<tr><td colspan="2" class="texto6"><b>R.U.T. :</b>&nbsp;&nbsp;' . number_format($row1["prov_rut"], 0, ',', '.') . '-' . $row1["prov_dv"] . '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>Nombre:</b>&nbsp;&nbsp;' . $pv . '</td></tr>';
        echo '<tr><td colspan="2" class="texto6"><b>Representante:</b>&nbsp;&nbsp;' . $row1["prov_nomcontacto"] . '&nbsp;&nbsp;&nbsp;<b>Teléfono:</b>&nbsp;&nbsp;' . $row1["prov_telefono"] . '<br><b>Mail Contacto:</b>&nbsp;&nbsp;' . $row1["prov_email"] . '</td></tr>';
        echo '</table>';
        echo '<table class="table4">';
        echo '<tr><td colspan="2" class="texto5" style="text-align:left;">2.- Datos del Solicitante:</td></tr>';
        echo '<tr><td colspan="2" class="texto6"><b>Nombre:</b>&nbsp;&nbsp;' . $row1["usuario_nombre"] . ' ' . $row1["usuario_apellidos"] . '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>Unidad Solicitante:</b>&nbsp;&nbsp;' . $row1["uni_nombre"] . '</td></tr>';
        echo '<tr><td colspan="2" class="texto6"><b>Teléfono:</b>&nbsp;&nbsp;' . $row1["usuario_telefono"] . ',&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>Email:</b>&nbsp;&nbsp;' . $row1["usuario_email"] . '<br><b>DIRECCIÓN DE ENTREGA PRODUCTOS:</b>&nbsp;&nbsp;' . $row1["bodega_dir"] . '</td></tr>';
        echo '</table>';
        echo '<table class="table4">';
        echo '<tr><td colspan="7" class="texto5" style="text-align:left;">3.- Detalle Productos Solicitados:</td></tr>';
        echo '<tr><th width="40">Codigo Interno</th><th width="40">Codigo CM</th><th width="170">Glosa</th><th width="20">Empaque</th><th width="55">Precio Unitario Neto Referencial</th><th width="20">Cantidad Requerida</th><th width="80">Orden de Compra MercadoPúblico</th></tr>';
        ...
        while ($row = mysql_fetch_array($data)) {
            $oc = $row["detmov_oc"];
            $sql2 = "select count(detmov_oc) as N, detmov_oc from detmovimiento where detmov_id='$id' AND detmov_oc='$oc' group by detmov_oc";
		 ...
		 <td style="color:red;font-weight:600;">' . $row["detmov_oc"] . '</td></tr>';
            ...
            if ($row2["N"] == 1) {
                $sum1 = ((float) $row["detmov_precio"] * (float) $row["detmov_cantidad"]);
                echo '<tr style="background-color:#ccffe6;"><td colspan="7" class="td4a"><b>Resumen Orden de Compra N°' . $row["detmov_oc"] . ': Artículos Solicitados: '.$row2["N"].', Total Neto (sin descuentos):&nbsp;&nbsp;&nbsp; $ ' . number_format($sum1, 0, ',', '.') . '</b></td></tr>';
            }
            if ($row2["N"] > 1) {
                $cont ++;
                $sum2 = $sum2 + ((float) $row["detmov_precio"] * (float) $row["detmov_cantidad"]);
            }
            if ($cont == $row2["N"]) {
                echo '<tr style="background-color:#ccffe6;"><td colspan="7" class="td4a"><b>Resumen Orden de Compra N° ' . $row["detmov_oc"] . ': Artículos Solicitados: '.$row2["N"].', Total Neto (sin descuentos):&nbsp;&nbsp;&nbsp; $ ' . number_format($sum2, 0, ',', '.') . '</b></td></tr>';
                $cont=0;
                $sum2=0;
            }
        }
        echo '<tr><td colspan="7" class="fondo"></td></tr>';
        echo '<tr><td colspan="6" class="td4">Total NETO (sin Descuentos):</td><td class="td4"><b> $ ' . number_format($sum, 0, ',', '.') . '</b></td></tr>';
...
$canvas->page_text(200, 770, "SigBod - Fecha de Impresion: " . date("d/m/Y H:i:s") . "  Página: {PAGE_NUM} de {PAGE_COUNT}", "Bold", 8, array(0, 0, 0));
